...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      let lineHeight = resolvedSize * 1.02;
      let maxMeasuredWidth = Math.max(1, ...lines.map(l => offCtx.measureText(l).width));
      let totalMeasuredHeight = lines.length * lineHeight;

      let scale = 1;
      if (maxMeasuredWidth > maxTextWidth) {
        scale = Math.min(scale, maxTextWidth / maxMeasuredWidth);
      }
      if (totalMeasuredHeight > maxTextHeight) {
        scale = Math.min(scale, maxTextHeight / totalMeasuredHeight);
      }

      if (scale < 1) {
        resolvedSize = Math.max(14, resolvedSize * scale);
        lineHeight = resolvedSize * 1.02;
        font = `${fontWeight} ${resolvedSize}px ${resolvedFamily}`;
        await waitForFonts(font);
        if (currentBuild !== buildId) return;
        offCtx.font = font;
        maxMeasuredWidth = Math.max(1, ...lines.map(l => offCtx.measureText(l).width));
        totalMeasuredHeight = lines.length * lineHeight;
      }

      const padding = Math.max(14, Math.ceil(resolvedSize * 0.12));
      ...
      offscreen.width = Math.ceil(maxMeasuredWidth + padding * 2);
      ...
      lines.forEach((line, index) => {
        offCtx.fillText(line, padding, padding + ascent + index * lineHeight);
      });
      ...
      const offsetX = align === 'center'
        ? Math.max(0, (width - offscreen.width) / 2)
        : 0;
      const offsetY = Math.max(0, (height - offscreen.height) / 2);
      ...
      const maxParticles = Math.max(1200, Math.min(6500, Math.floor((width * height) / 75)));
